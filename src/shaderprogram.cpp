...
void ShaderProgram::create(const char *vertfile, const char *fragfile)
{
    ...
    unifGrey       =context->glGetUniformLocation(prog,"u_Grey");
}
...
void ShaderProgram::setupTexture(){
    ...
    if(unifGrey !=-1){
        useMe();
        context->glGenTextures(1,&greyHandle);
        context->glActiveTexture(GL_TEXTURE2);
        context->glBindTexture(GL_TEXTURE_2D, greyHandle);
        int w,h;
        unsigned char* grey = SOIL_load_image("../final2/image/minecraft_textures_all_grey_grass.png", &w, &h, 0, SOIL_LOAD_RGBA);
        printf("SOIL fire texture results: '%s'\n", SOIL_last_result());
        context->glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, w, h, 0, GL_RGBA, GL_UNSIGNED_BYTE, grey);
        glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_NEAREST);
        glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_NEAREST);
        SOIL_free_image_data(grey);
        //glEnable (GL_BLEND); glBlendFunc (GL_SRC_ALPHA, GL_ONE_MINUS_SRC_ALPHA);
    }
}
