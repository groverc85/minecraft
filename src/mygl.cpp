    ...
    if(chunkManager.renderList.size()!=0){
        for(Chunk* chunk:chunkManager.renderList){
            if(chunk->isActive()==true){
             ...
             glUniform1i(prog_tex.unifGrey,2);
             glActiveTexture(GL_TEXTURE2);
             glBindTexture(GL_TEXTURE_2D, prog_tex.greyHandle);
             prog_tex.draw(*chunk);
...
void MyGL::setWoodOnFire(){
    ...
    for(int i=0;i<size;i++){
        ...
        Block upBlock;
        upBlock.pos=std::make_tuple(x, y+1, z);
        if(rand%5==3){
        setNearOnFire(upBlock);
        }
        Block downBlock;
...
void MyGL::timerUpdate()
{
    ...
    /// ------------------------------------------------------------
       ...
       ///--------------added by Yi Huang-------------------------
        float x=fabs(sin(timecount/200.0f))*0.8;
        float z=0.2+fabs(sin(timecount/200.0f))*0.8;
    ...
    if (time_count / 1000 - oldTime >= 3)
    {
        ...
        for (int x = floor(gl_camera.eye[0]/16.0f)*16 - 16; x < floor(gl_camera.eye[0]/16.0f)*16 + 16; x++)
            for(int z = floor(gl_camera.eye[2]/16.0f)*16 - 16; z < floor(gl_camera.eye[2]/16.0f)*16 + 16; z++)
            {
                ...
                scene.containBlock[pos] = true;
                scene.scene[pos] = SnowFlake;
            }

        oldTime = time_count / 1000;
        //printf("oldTime = %d\n", oldTime);
